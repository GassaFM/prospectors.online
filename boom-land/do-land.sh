#!/bin/bash
cd "${0%/*}"
../land/refresh-logs-auction "https://eos.dfuse.eosnation.io/graphql" "account:prospectorsq (action:endauction OR action:endlocexpr OR action:endlocsale OR action:mkfreeloc)" || exit 1
