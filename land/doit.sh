#!/bin/bash
cd "${0%/*}"
./refresh-logs-auction "https://eos.dfuse.eosnation.io/graphql" "account:prospectorsc (action:endauction OR action:endlocexpr OR action:endlocsale OR action:mkfreeloc)" || exit 1
